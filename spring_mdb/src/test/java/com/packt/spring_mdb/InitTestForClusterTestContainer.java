...
@SpringBootTest
@Testcontainers
public abstract class InitTestForClusterTestContainer {

  static Logger logger = LoggerFactory.getLogger(InitTestForClusterTestContainer.class);
  ...
  static GenericContainer<?> mongoDBContainer1 = new GenericContainer<>("mongo:latest").withNetwork(mongoDbNetwork).withCommand("mongod", "--replSet", "rs0", "--port", "27017", "--bind_ip", "localhost,mongo1").withNetworkAliases("mongo1").withExposedPorts(27017)
  ...
  static GenericContainer<?> mongoDBContainer2 = new GenericContainer<>("mongo:latest").withNetwork(mongoDbNetwork).withCommand("mongod", "--replSet", "rs0", "--port", "27017", "--bind_ip", "localhost,mongo2").withNetworkAliases("mongo2").withExposedPorts(27017);

  static GenericContainer<?> mongoDBContainer3 = new GenericContainer<>("mongo:latest").withNetwork(mongoDbNetwork).withCommand("mongod", "--replSet", "rs0", "--port", "27017", "--bind_ip", "localhost,mongo3").withNetworkAliases("mongo3").withExposedPorts(27017);
  ...
  static void startContainer() throws IOException, InterruptedException {
    ...
    logger.info("3 Mongo containers started, now importing data");
  ...
  static void setMongoDbProperties(DynamicPropertyRegistry registry) {
    registry.add("spring.data.mongodb.uri", () -> "mongodb://" + mongoDBContainer1.getHost() + ":" + mongoDBContainer1.getMappedPort(27017) + "/?directConnect=true");
  }
